...
from app.shared.nlp import embed

SORT_BY = {"Latest": NEWEST_AND_TRENDING, "Trending": LATEST_AND_TRENDING}
# project = {K_EMBEDDING: 0, K_TEXT:0, K_ID: 0}
BEAN_BODY_FIELDS = {K_URL: 1, K_TAGS: 1, K_SUMMARY: 1}
BEAN_HEADER_FIELDS = {K_URL: 1, K_TITLE: 1, K_KIND: 1, K_IMAGEURL: 1, K_AUTHOR: 1, K_SOURCE: 1, K_SITE_NAME: 1, K_SITE_BASE_URL: 1, K_CREATED: 1, K_LIKES: 1, K_COMMENTS: 1, K_SHARES: 1}
BEAN_SEARCH_FIELDS = {K_URL: 1, K_TAGS: 1, K_EMBEDDING: 1}

db: Beansack = None

def initiatize(db_conn: str, db_name: str = "beansack"):
    global db
...
#     return db.query_beans(filter=filter, sort_by=SORT_BY[sort_by], skip=start, limit=limit, project=BEAN_HEADER_FIELDS)
...
def get_beans_for_barista(
    ...
    if barista.query_urls: return db.query_beans(filter=filter, sort_by=sort_by, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)
    if barista.query_embedding: return db.vector_search_beans(embedding=barista.query_embedding, similarity_score=barista.query_distance or DEFAULT_ACCURACY, filter=filter, distinct_field=K_CLUSTER_ID, sort_by=sort_by, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)
    if barista.query_tags or barista.query_sources: 
        if filter_category: return db.vector_search_beans(embedding=get_barista(filter_category, BARISTA_EMBEDDING_FIELDS).query_embedding, similarity_score=DEFAULT_ACCURACY, filter=filter, distinct_field=K_CLUSTER_ID, sort_by=sort_by, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)
...
#     )    
#     project = {"summary": 1}
#     project.update(BEAN_HEADER_FIELDS)
#     return db.query_beans_per_cluster(filter=filter, sort_by=SORT_BY[sort_by], skip=start, limit=limit, project=project)
...
def get_barista_tags(barista: Barista, start: int, limit: int):
    ...
    if barista.query_embedding: return db.vector_search_tags(embedding=barista.query_embedding, similarity_score=barista.query_distance or DEFAULT_ACCURACY, filter=filter, skip=start, limit=limit)
...
@cached(max_size=CACHE_SIZE, ttl=HALF_HOUR)
def get_beans_for_source(source: str, tags: str|list[str]|list[list[str]], kinds: str|list[str], categories: str, last_ndays: int, sort_by, start: int, limit: int):
    """Retrieves news articles, social media posts, blog articles from the feed source"""  
    sort_by = SORT_BY[sort_by]
    filter=_create_filter(tags, kinds, source, None, last_ndays, None)
    if categories: return db.vector_search_beans(embedding=get_barista(categories, BARISTA_EMBEDDING_FIELDS).query_embedding, similarity_score=DEFAULT_ACCURACY, filter=filter, distinct_field=K_CLUSTER_ID, sort_by=sort_by, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)
    return db.query_beans(filter=filter, distinct_field=K_CLUSTER_ID, sort_by=sort_by, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)   
...
def search_beans(query: str, accuracy: float, tags: str|list[str]|list[list[str]], kinds: str|list[str], sources: str|list[str], last_ndays: int, sort_by, start: int, limit: int):
    ...
    if query: return db.vector_search_beans(embedding=embed(query), similarity_score=accuracy, filter=filter, skip=start, limit=limit, project=BEAN_HEADER_FIELDS)    
...
def count_search_beans(query: str, accuracy: float, tags: str|list[str]|list[list[str]], kinds: str|list[str], sources: str|list[str], last_ndays: int, limit: int) -> int:
    ...
    if query: return db.count_vector_search_beans(embedding=embed(query), similarity_score=accuracy, filter=filter, limit=limit)  
...
def get_related(url: str, tags: str|list[str]|list[list[str]], kinds: str|list[str], sources: str|list[str], last_ndays: int, limit: int):
    filter = _create_filter(tags, kinds, sources, None, last_ndays, None)
    return db.query_related_beans(url=url, filter=filter, limit=limit) 
...
@cached(max_size=CACHE_SIZE, ttl=ONE_HOUR)
def get_barista(id: str, project: dict = BARISTA_DEFAULT_FIELDS) -> Barista|None:
    barista = db.baristas.find_one(filter={K_ID: id}, projection=project)
    if barista: return Barista(**barista)

@cached(max_size=CACHE_SIZE, ttl=FOUR_HOURS)
def get_baristas(ids: list[str], project: dict = BARISTA_DEFAULT_FIELDS) -> list[Barista]:
    if not ids: return
